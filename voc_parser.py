...
#					test_files.append(line.strip() + '.jpg')
			...
			try:
				...
				for element_obj in element_objs:
					...
					if class_name not in class_mappingNameToId:
						class_mappingNameToId[class_name] = len(class_mappingNameToId) 
						class_mappingIdToName[len(class_mappingNameToId)-1] = class_name
					...
					annotation_data['bboxes'].append(
						{'class': class_name, 'x1': x1, 'x2': x2, 'y1': y1, 'y2': y2, 'difficult': difficulty, 'x':x, 'y':y, 'w':w, 'h':h})
		...
		# if there are a bg class make it last index
			...
			lastName = class_mappingIdToName[len(class_mappingNameToId)-1]
